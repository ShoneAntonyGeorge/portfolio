...
function Expanded({ setExpand, journey }) {
  return (
    <div className="expandSlideClass fixed top-0 z-1000 flex h-screen w-screen items-center justify-center">
      <div className="relative z-10 flex max-h-95/100 min-h-6/7 w-95/100 flex-col overflow-hidden rounded-3xl border-3 border-[#5a045d] bg-white shadow-lg lg:h-6/7 lg:w-4/5 lg:flex-row">
        <div
          onClick={() => setExpand("")}
        ...
        <div className="flex h-2/10 w-full items-center rounded-t-3xl border-b-3 p-5 lg:h-[110%] lg:w-fit lg:max-w-1/3 lg:rounded-t-none lg:rounded-l-3xl lg:border-r-3 lg:border-r-[#5a045d]">
          <span className="w-fit min-w-6/10 bg-gradient-to-r from-[#5a045d] to-[#c90098] bg-clip-text text-xl font-bold text-transparent lg:text-5xl">
            {journey.title}
          </span>
        </div>
        ...
        <div className="flex h-full w-full flex-col overflow-y-auto rounded-r-3xl p-5 lg:w-7/10 lg:p-15">
          ...
          <ul className="flex list-disc flex-col gap-4 p-5">
            {journey.points.map((point, index) => (
              <li
                key={index}
                className="text-sm font-semibold text-gray-700 lg:text-lg"
              >
                {point}
              </li>
            ))}
